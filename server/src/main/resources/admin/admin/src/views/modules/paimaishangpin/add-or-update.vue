          ...
          <el-form-item class="input" label="商品名称" prop="shangpinmingcheng">
              <el-input v-model="ruleForm.shangpinmingcheng" 
                placeholder="商品名称" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="商品类型" prop="categories">
          <el-select v-model="ruleForm.categories" placeholder="请选择商品类型">
            <el-option
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `auction-items/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
